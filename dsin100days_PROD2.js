...
export default function() {
  ...
  let params = {'Content-Type': 'application/x-www-form-urlencoded ', timeout: 180000};
  ...
  group('v1 Refactored Jupyter Hub load testing', function() {
    ...
    group('login ', function() {
      ...
      check(res, { "Your server is starting up..":  (r) => r.body.includes('Server not running')});
      check(res, { "Server not running..":  (r) => r.body.includes('<button id="logout" class="btn btn-sm navbar-btn">Logout</button>')});
...
/*    
      let res = http.get('https://dsin100days.refactored.ai//hub/login',params);
...
/*
    group('Jupyter hub heart-beat', function() {
      ...
      if (! res.body.includes('data-notebook-name="01-Basic_data_types_and_operators.ipynb"')) {
        ...
        let res2 = http.get("https://dsin100days.refactored.ai/user/" + user.username + "/git-pull?repo=https%3A%2F%2Fgithub.com%2Fcolaberry%2Fdsin100days&branch=master&subPath=content%2F02-Python_for_Data_Scientists%2F03-Data_Structures_in_python%2F01-Basic_data_types_and_operators.ipynb&redirects=1");
        sleep(180);
        let res3 = http.get("https://dsin100days.refactored.ai/user/" + user.username + "/git-pull/api?repo=https%3A%2F%2Fgithub.com%2Fcolaberry%2Fdsin100days&branch=master");
